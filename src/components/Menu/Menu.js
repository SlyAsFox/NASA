...
class Menu extends Component {
    ...
    state = {
        selectValue: 'aim',
        satellite: satelliteInfo[1]
    };

    findSatellite =  satelliteInfo.find( ( item ) => {
        return item.name === this.state.selectValue
    });
    ...
    handleChange = e => {
        ...
        this.setCurrentSatellite();
    };

    setCurrentSatellite = () => {
        let result = this.findSatellite();
        this.setState({
            satellite: result
            }
        );
  ...
  return (
    <div className="menu">
        <div className='selector'>
            <span className="title-span">Satellite:&nbsp;&nbsp;&nbsp;</span>
            <select
            ...
            >
                ...
                {satelliteInfo.map(item => <option value={item}>{item.name}</option>)}
            </select>
        </div>
        <div className='title-info'>
            { this.state.satellite.name ? <span> <span className="title-span">Name:</span>  {this.state.satellite.name}</span> : null }
            { this.state.satellite.type ? <span><span className="title-span">Type:</span>  {this.state.satellite.type}</span> : null }
            { this.state.satellite.origin ? <span><span className="title-span">Origin:</span>  {this.state.satellite.origin}</span> : null }
            { this.state.satellite.mission ? <span><span className="title-span">Mission:</span>  {this.state.satellite.mission}</span> : null }
            { this.state.satellite.missionDuration ? <span><span className="title-span">Mission duration:</span>  {this.state.satellite.missionDuration}</span> : null }
            { this.state.satellite.website ? <a href={this.state.satellite.website}>More info...</a> : null}
        </div>
    </div>
  );
};}
